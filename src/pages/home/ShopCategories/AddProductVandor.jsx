...
const AddProductVendor = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const { data } = await axiosInstance.post("/vendor/products", formData);
      ...
      if (data.success) {
        ...
        setFormData((prev) => ({
          ...
          marketName: "",
          marketDescription: "",
          items: [{ item_name: "", unit: "kg", current_price: 0, prices: [], description: "", image: "" }],
        }));
      } else {
  ...
  return (
    <div className="pt-16 md:pt-24 px-4 md:px-10 lg:px-20 min-h-screen bg-[#f5f0e1] flex items-center justify-center">
      <div className="w-full max-w-4xl bg-white p-6 md:p-10 rounded-xl shadow-lg">
        <h2 className="text-2xl md:text-3xl font-bold mb-6 text-center text-gray-800">Add New Vendor Product</h2>
        <form onSubmit={handleSubmit} className="space-y-6">
          {/* Readonly Inputs */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <input type="email" name="vendorEmail" value={formData.vendorEmail} readOnly className="border border-gray-300 p-3 rounded-lg w-full bg-gray-100 cursor-not-allowed focus:outline-none focus:ring-2 focus:ring-blue-400 transition" />
            <input type="text" name="vendorName" value={formData.vendorName} readOnly className="border border-gray-300 p-3 rounded-lg w-full bg-gray-100 cursor-not-allowed focus:outline-none focus:ring-2 focus:ring-blue-400 transition" />
          </div>
          ...
          <div className="flex justify-center">
            <button type="submit" className="bg-blue-500 hover:bg-blue-600 text-white px-8 py-3 rounded-lg transition-colors duration-200 shadow-md">Add Product</button>
          </div>
        </form>
      </div>
    </div>
